...
class OMDbService {
  ...
  private checkRateLimit(): void {
    if (this.requestCount >= OMDB.RATE_LIMIT * 0.9) {
      Logger.warn(`⚠️ Approaching OMDb API limit: ${this.requestCount}/${OMDB.RATE_LIMIT}`);
    }
  }
  ...
  async searchMovies(params: SearchParams): Promise<any> {
    ...
    try {
      const response = await this.client.get<OMDbSearchResponse>('', { params: apiParams });
   ...
   */
  async getMovieDetails(id: string, plot: 'short' | 'full' = 'full'): Promise<any> {
    // Check cache first
